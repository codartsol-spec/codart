...
(function () {
    ...
    function getModal()  { return document.getElementById('calendly-modal');  }
    function getWidget() { return document.getElementById('calendly-widget'); }
    ...
    window.openCalendly = function (url) {
        ...
        // Swap URL and re-init widget for the correct event type
        const targetUrl = url || 'https://calendly.com/codart-sol/30min';
        if (widget) {
            widget.setAttribute('data-url', targetUrl);
    ...
    window.handleOverlayClick = function (e) {
        if (e.target === getModal()) window.closeCalendly();
    };

    document.addEventListener('keydown', function (e) {
        const modal = getModal();
        if (e.key === 'Escape' && modal && modal.classList.contains('open')) {
            window.closeCalendly();
        }
    });
})();
